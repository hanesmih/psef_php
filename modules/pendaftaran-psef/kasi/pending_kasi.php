...
<script>
        ...
        $('#zero_config').on('xhr.dt', function ( e, settings, json, xhr ) {
            ...
            "dataSrc": function ( json ) {
              ...
              for (let i = 0; i < json.data.length; i++) {
                let info = (role != "Psef.Supervisor") ? `<p>${json.data[i].statusName}</p>` :
                ...
                let action = (role != "Psef.Supervisor") ? `<button onclick="view_data('${json.data[i].permohonanId}')" class="btn btn-xs btn-block btn-info">Lihat Detail Data</button>` :
            ...
            "data": function ( d ) {
              return configurePermohonanRequest(d, 'Permohonan/KepalaSeksiPending');
            }
            }
        });
    });
    ...
    function reject_data(id){
      permohonanKembalikan(id, url_api_x + 'Permohonan/KepalaSeksiKembalikan', accesstoken);
    }

    function process_data(id){
      permohonanSetujui(id, url_api_x + 'Permohonan/KepalaSeksiSetujui', accesstoken);
    }
</script>
