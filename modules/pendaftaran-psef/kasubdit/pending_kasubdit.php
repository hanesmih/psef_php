        ...
        $('#zero_config').on('xhr.dt', function ( e, settings, json, xhr ) {
            ...
            "dataSrc": function ( json ) {
              ...
              for (let i = 0; i < json.data.length; i++) {
                let info = (role != "Psef.Timja") ? `<p>${json.data[i].statusName}</p>` :
                ...
                let action = (role != "Psef.Timja") ? `<button onclick="view_data('${json.data[i].permohonanId}')" class="btn btn-xs btn-block btn-info">Lihat Detail Data</button>` :
